...
async def blockscout_holders(a,cursor=None,limit=10):
    d=await blockscout_request(c["chain_id"],f"/tokens/{a}/holders",cursor)
...
async def evm_holders(a,c):
    ...
    first=await blockscout_holders(a,None,10)
    if first.get("items"):
...
async def evm_holder_next(a,c,cursor):
    d=await blockscout_holders(a,cursor,10)
...
async def solana_token(a):
    ...
    for u in urls:
        try:
            async with aiohttp.ClientSession() as s:
                supply=await rpc(s,u,"getTokenSupply",[a]);asset=await rpc(s,u,"getAsset",[a,{"displayOptions":{"showFungible":True}}])
            if supply is None:continue
            v=supply.get("value",{});d=int(v.get("decimals",0));raw=int(v.get("amount","0"));md=((asset or {}).get("content") or {}).get("metadata") or {}
            return {"family":"solana","chain":"Solana","contract":a,"name":md.get("name") or"SPL Token","symbol":md.get("symbol") or"","decimals":d,"total_supply":raw/(10**d if d else 1)}
        except Exception as e:last=str(e)
    raise RuntimeError(last)
...
async def solana_all_holders(a):
    ...
    url=f"https://solana-mainnet.g.alchemy.com/v2/{key}";owners={};cursor=None;token_accounts=0
    async with aiohttp.ClientSession() as s:
        for _ in range(200):
            ...
            rows=d.get("token_accounts") or d.get("tokenAccounts") or [];token_accounts+=len(rows)
            for x in rows:
            ...
            if token_accounts>=200000:raise RuntimeError("Solana token has more than 200,000 token accounts; safety limit reached.")
    items=[{"address":o,"raw":v,"value":str(v)} for o,v in owners.items() if v>0];items.sort(key=lambda x:x["raw"],reverse=True)
    return {"items":items,"total":len(items),"source":"Alchemy Solana DAS (aggregated by wallet owner)","token_accounts":token_accounts,"page_size":10}
